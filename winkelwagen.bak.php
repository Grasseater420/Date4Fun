...
<html>
  <head>
    <?php
      ...
      renderHead('Date4Fun Winkelwagen');
      renderNavbar();
      // renderJumbotron();
    ...
    <?php
      ...
      function winkelWagenOverzicht($db) {
        // Hier staat niets voor zo ver ik kan zien.
      ...
      if (count($_SESSION['winkelwagen']) >= 1) {
        ...
        $sql = "SELECT IFNULL(events.titel, membership.omschrijving) as titel, producten.prijs, producten.product_id
          ...
          LEFT JOIN membership
            ON membership.membership_id=producten.membership_id WHERE producten.product_id IN ($str)";

        $result = mysqli_query($db, $sql);

        $sql = "SELECT membership.korting as waarde
          FROM membership
          INNER JOIN members
            ON members.membership_id=membership.membership_id
              WHERE members.gebruiker_id = '".$_SESSION['gebruikers_id']."'";
